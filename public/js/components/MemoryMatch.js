...
function placementHonour(result) {
  ...
  const details = [{ icon: '👑', title: 'New Relic Recall Champion!', copy: 'The memory crown is yours!' }, { icon: '🥈', title: 'A Silver Recall!', copy: 'You have claimed second place!' }, { icon: '🥉', title: 'Top Three Recall!', copy: 'A place in the realm’s legends is yours!' }][position];
...
export function openMemoryMatch({ scores: initialScores = [], onScore = async () => initialScores } = {}) {
  const overlay = createModal(`<div class="memory-modal"><div class="blocks-heading"><div><p class="blocks-kicker">THE TAVERN GAME TABLE</p><h2 class="modal-title">✦ Relic Recall ✦</h2></div><button class="modal-close" id="memory-close">×</button></div><p class="blocks-intro">Match guild heroes and relics before the sands run out. Each victory grows the board.</p><div class="memory-stats"><div><span>Level</span><strong id="memory-level">1</strong></div><div><span>Pairs</span><strong id="memory-pairs">0</strong></div><div><span>Time</span><strong id="memory-time">0</strong></div><div><span>Score</span><strong id="memory-score">0</strong></div></div><div class="memory-board-wrap"><div class="memory-board" id="memory-board"></div><div class="memory-game-over" id="memory-game-over" role="status"></div></div><div class="memory-actions"><button class="blocks-start" id="memory-start">▶ Begin Recall</button><span id="memory-message">Match every pair to advance.</span></div><section class="runefall-scoreboard"><div class="blocks-label">🏆 Relic Recall — Top Three</div><div id="memory-scores">${scoreRows(initialScores)}</div></section></div>`, cleanup);
  const board = overlay.querySelector('#memory-board'), start = overlay.querySelector('#memory-start'), message = overlay.querySelector('#memory-message'), scoreBoard = overlay.querySelector('#memory-scores'), gameOver = overlay.querySelector('#memory-game-over');
  ...
  function render() { const setup = LEVELS[Math.min(level - 1, LEVELS.length - 1)]; board.style.setProperty('--memory-cols', setup.cols); board.innerHTML = cards.map((card, index) => `<button class="memory-card ${card.open ? 'open' : ''} ${card.matched ? 'matched' : ''}" data-card="${index}" draggable="false" ${card.matched ? 'disabled' : ''}><span class="memory-card-back">✦</span><img src="${card.image}" alt="${escapeHtml(card.name)}" draggable="false"><small>${escapeHtml(card.name)}</small></button>`).join(''); board.querySelectorAll('[data-card]').forEach(button => { button.addEventListener('click', () => flip(Number(button.dataset.card))); button.addEventListener('dragstart', event => event.preventDefault()); }); }
  function beginLevel() { const setup = LEVELS[Math.min(level - 1, LEVELS.length - 1)]; const chosen = shuffle(CARD_POOL).slice(0, setup.pairs); cards = shuffle(chosen.flatMap(card => [{ ...card }, { ...card }])); pairs = 0; open = []; locked = false; seconds = Math.max(14, 42 - level * 5); clearInterval(timer); gameOver.classList.remove('show'); timer = setInterval(() => { seconds--; update(); if (seconds <= 0) end(); }, 1000); message.textContent = `Level ${level}: find ${setup.pairs} pairs before time runs out.`; start.textContent = '↻ Restart Run'; render(); update(); }
  ...
  async function end() { clearInterval(timer); board.querySelectorAll('button').forEach(button => button.disabled = true); message.textContent = `The sands fall. You reached level ${level} with ${score} renown.`; start.textContent = '▶ Begin Recall'; gameOver.innerHTML = `<span>⌛ Time’s Up!</span><strong>${score.toLocaleString()} renown</strong><small>Reached Level ${level}</small><button class="blocks-start" type="button">↻ Play Again</button>`; gameOver.classList.add('show'); gameOver.querySelector('button').onclick = startRun; if (!submitted && score > 0) { submitted = true; try { const scoreResult = await onScore({ score, level, pairs }); scores = scoreResult?.scores || scoreResult; scoreBoard.innerHTML = scoreRows(scores); const honour = placementHonour(scoreResult); if (honour) gameOver.querySelector('button').insertAdjacentHTML('beforebegin', honour); } catch {} } }
  function cleanup() { clearInterval(timer); }
  overlay.querySelector('#memory-close').onclick = () => { cleanup(); closeModal(); };
  function startRun() { level = 1; score = 0; submitted = false; beginLevel(); }
